  ...
  while (1) {
    ...
    if (currentState == RobotState::TRACE_BLACK) {
      ...
      if (blue_detect_count >= SWITCH_CONFIRM_COUNT) {
        lap_count++;
        printf("青ライン検知！ 青トレースモードに切り替え (検知回数: %d)\n", lap_count);
    ...
    } else { // currentState == RobotState::TRACE_BLUE
    ...
    const char* trace_edge_mode; // 表示用の文字列ポインタ
    
    if (lap_count == 0 || lap_count == 1 || lap_count == 3) {
      // 【右エッジトレース】
      ...
      leftPower = basePower - static_cast<int>(turn);
      rightPower = basePower + static_cast<int>(turn);
      trace_edge_mode = "Left "; // 文字幅を合わせるためのスペース
    }
